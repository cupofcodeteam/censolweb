@extends('partials.layout')

@section('title','CENSOL | Comercial')

@section('content')

<div id="blog-breadcrumb" class="text-center page-breadcrumb">
  <div class="container">
  </div>
</div><!--/.page-breadcrumb-->

<div id="apostilla" class="informacion-1">
  <div class="welcome  text-center">
    <h1> INTERCAMBIO COMERCIAL</h1>
  </div>
  <ol class="breadcrumb">
    <li class="breadcrumb-item"><a href="/"> Inicio</a></li>
    <li class="breadcrumb-item"> Comercialización </li>
    <li class="breadcrumb-item active"> Intercambio comercial </li>
  </ol>
  <div class="container">
    <div class="row padding-bottom">
      <div class="col-md-6 ">
        <p>CENSOL GUAYANA ofrece la oportunidad a empresas de la región a hacer intercambio comercial o convenio por clases en nuestras instalaciones o a domicilio en sus oficinas. Para mayor información contáctenos y visítenos en nuestras instalaciones en el CC Biblos Center de Unare 2 en Puerto Ordaz.</p>
        <a href="/nosotros/contacto" target="_blank" class="btn btn-primary section-btn appear-top">Contáctenos</a>
      </div>
      <div class="col-md-6">
        <img class="appear-right img-responsive" src="/images/quienes-somos/intercambioC.jpeg" alt="" width="500" height=auto>
      </div>
    </div>
  </div><!--/#main-blog-->
</div>
@endsection
